...
#SENSOR_OFF = state.get(sensor_off)
#SENSOR_ON = state.get(sensor_on)
#SENSOR_PRESENCE = state.get(sensor_presence)
#HEATING_FROM_HOUR = int(state.get(heating_from_hour))
#HEATING_TO_HOUR = int(state.get(heating_to_hour))
#HIGH_TEMP = int(state.get(high_temp))
#LOW_TEMP = int(state.get(low_temp))
#
...
#bool_presence = True if SENSOR_PRESENCE is None else not hass.states.is_state(
#    SENSOR_PRESENCE, STATE_OFF,
#)
