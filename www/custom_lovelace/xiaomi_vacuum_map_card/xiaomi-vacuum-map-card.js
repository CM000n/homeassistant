...
class XiaomiVacuumMapCard extends LitElement {
  ...
  setConfig(config) {
    ...
    if (config.map_image && config.map_camera) {
      throw new Error(
        "Only one of following properties is allowed: map_image or map_camera",
      );
    }
    ...
    if (!config.camera_calibration) {
      if (
        ...
        !Array.isArray(config.calibration_points)
      ) {
        throw new Error(
          "Missing configuration: calibration_points or camera_calibration",
        );
      }
      ...
      if (!config.map_camera) {
        throw new Error(
          "Invalid configuration: map_camera is required for camera_calibration",
        );
      }
    }
    ...
    if (
      ...
      (config.zones.length === 0 &&
        this.modes.includes(texts[this._language][zones]))
    ) {
      this.modes.splice(this.modes.indexOf(texts[this._language][zones]), 1);
    }
    if (config.default_mode) {
      if (
        !availableModes.has(config.default_mode) ||
        !this.modes.includes(availableModes.get(config.default_mode))
      ) {
        throw new Error("Invalid default mode: " + config.default_mode);
      }
      this.defaultMode = this.modes.indexOf(
        availableModes.get(config.default_mode),
      );
    } else {
      this.defaultMode = -1;
    }
  ...
  getConfigurationMigration(config) {
    ...
    return html` <ha-card id="xiaomiCard" style="padding: 16px">
      <div class="card-header" style="padding: 8px 0 16px 0;">
        <div class="name">Xiaomi Vacuum Map card</div>
      </div>
  ...
  render() {
    ...
    const modesDropdown = this.modes.map(
      (m) => html`<paper-item>${m}</paper-item>`,
    );
    const rendered = html` ${style}
      <ha-card id="xiaomiCard">
        <div id="mapWrapper">
          <div id="map">
            <img
              id="mapBackground"
              @load="${() => this.calculateScale()}"
              src="${this.map_image}"
            />
            <canvas
              id="mapDrawing"
        ...
        ${this.missingCameraAttribute
          ? html`<div style="padding: 5px;">
              ...
              <p>
                Enable calibration_points in camera entity or disable
                camera_calibration
              </p>
            </div>`
          : html`<div class="dropdownWrapper">
                <paper-dropdown-menu
                  label="${texts[this._language][mode]}"
                  @value-changed="${(e) => this.modeSelected(e)}"
                  class="vacuumDropdown"
                  selected="${this.defaultMode}"
                >
                  <paper-listbox
              ...
              <p class="buttonsWrapper">
                <span id="increaseButton" hidden
                  ><mwc-button
                    @click="${() => this.vacuumZonedIncreaseButton()}"
                    >${texts[this._language][repeats]}
                    ${this.vacuumZonedCleanupRepeats}</mwc-button
                  ></span
                >
                <mwc-button
                  class="vacuumRunButton"
                  @click="${() => this.vacuumStartButton(true)}"
                  >${texts[this._language][run]}</mwc-button
                >
              </p>
              <div id="toast">
                <div id="img">
                ...
                <div id="desc">${texts[this._language][confirmation]}</div>
              </div>`}
      </ha-card>`;
    if (this.getMapImage()) {
      this.calculateScale();
  ...
  onMouseDown(e) {
    ...
    if (this.mode === 1) {
      ...
      const { selected, shouldDelete, shouldResize } =
        this.getSelectedRectangle(pos.x, pos.y);
      ...
      if (selectedZone >= 0) {
        if (this.selectedZones.includes(selectedZone)) {
          this.selectedZones.splice(
            this.selectedZones.indexOf(selectedZone),
            1,
          );
        } else {
          if (
            this.selectedZones.length < 5 ||
            this._config.ignore_zones_limit
          ) {
  ...
  onMouseUp(e) {
    this.isMouseDown = false;
    if (
      this.selectedRectangle >= 0 ||
      this.mode !== 2 ||
      (this.mode === 2 &&
        this.rectangles.length >= 5 &&
        !this._config.ignore_zones_limit)
    ) {
      this.selectedRectangle = -1;
      this.drawCanvas();
  ...
  onMouseMove(e) {
    if (this.isMouseDown && this.mode === 2) {
      ...
      if (this.selectedRectangle < 0) {
        ...
        this.rectangles[this.selectedRectangle].x =
          this.currRectangle.w + x - this.currRectangle.x;
        this.rectangles[this.selectedRectangle].y =
          this.currRectangle.h + y - this.currRectangle.y;
      }
      this.drawCanvas();
    }
  ...
  modeSelected(e) {
    ...
    this.getPredefinedZonesIncreaseButton().hidden =
      this.mode !== 3 && this.mode !== 2;
    this.drawCanvas();
  }
  ...
  drawCanvas() {
    ...
    if (this._config.debug) {
      ...
      if (this._config.camera_calibration) {
        calibration_points =
          this._hass.states[this._config.map_camera].attributes
            .calibration_points;
      }
      for (const calibration_point of calibration_points) {
        const { x, y } = this.convertVacuumToMapCoordinates(
          calibration_point.vacuum.x,
          calibration_point.vacuum.y,
        );
    ...
    if (this.mode === 1 && this.currPoint.x != null) {
      ...
      if (this.isMouseDown && this.selectedRectangle < 0) {
        ...
        context.lineWidth = 1;
        context.rect(
          this.currRectangle.x,
          this.currRectangle.y,
          this.currRectangle.w,
          this.currRectangle.h,
        );
        context.stroke();
      }
    } else if (this.mode === 3) {
      for (let i = 0; i < this._config.zones.length; i++) {
        ...
        for (const rect of zone) {
          const { x, y, w, h } = this.convertVacuumToMapZone(
            rect[0],
            rect[1],
            rect[2],
            rect[3],
  ...
  getSelectedRectangle(x, y) {
    ...
    for (let i = this.rectangles.length - 1; i >= 0; i--) {
      ...
      if (
        Math.pow(x - rect.x - rect.w, 2) + Math.pow(y - rect.y - rect.h, 2) <=
        64
      ) {
        selected = i;
        shouldResize = true;
  ...
  vacuumGoToPoint(debug) {
    const mapPos = this.convertMapToVacuumCoordinates(
      this.currPoint.x,
      this.currPoint.y,
    );
    if (debug && this._config.debug) {
      ...
      this._hass
        .callService(this.service_domain, this.service_method, {
  ...
  vacuumStartZonedCleanup(debug) {
    ...
    for (const rect of this.rectangles) {
      zone.push(
        this.convertMapToVacuumRect(rect, this.vacuumZonedCleanupRepeats),
      );
    }
    if (debug && this._config.debug) {
  ...
  convertMapToVacuumRect(rect, repeats) {
    ...
    const xy2 = this.convertMapToVacuumCoordinates(
      rect.x + rect.w,
      rect.y + rect.h,
    );
  ...
  convertMapToVacuumCoordinates(mapX, mapY) {
    const { x, y } = this.coordinatesConverter.convertAB(
      mapX / this.imageScale,
      mapY / this.imageScale,
    );
    return { x: Math.round(x), y: Math.round(y) };
  }

  convertVacuumToMapZone(vacuumX1, vacuumY1, vacuumX2, vacuumY2) {
    const { x: x1, y: y1 } = this.convertVacuumToMapCoordinates(
      vacuumX1,
      vacuumY1,
    );
    const { x: x2, y: y2 } = this.convertVacuumToMapCoordinates(
      vacuumX2,
      vacuumY2,
    );
  ...
  convertVacuumToMapCoordinates(vacuumX, vacuumY) {
    const { x: vX, y: vY } = this.coordinatesConverter.convertBA(
      vacuumX,
      vacuumY,
    );
