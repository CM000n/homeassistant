...
class XiaomiVacuumMapCard extends LitElement {
	...
	setConfig(config) {
		...
		if (
			...
			(config.zones.length === 0 && this.modes.includes(texts[this._language][zones]))
		) {
  ...
  - vacuum:
					...
					: html`<div class="dropdownWrapper">
							...
							<p class="buttonsWrapper">
								<span id="increaseButton" hidden
									><mwc-button @click="${() => this.vacuumZonedIncreaseButton()}"
	...
	onMouseDown(e) {
		...
		if (this.mode === 1) {
			...
			const { selected, shouldDelete, shouldResize } = this.getSelectedRectangle(
				pos.x,
				pos.y,
			);
	...
	drawCanvas() {
		...
		if (this._config.debug) {
			...
			if (this._config.camera_calibration) {
				calibration_points =
					this._hass.states[this._config.map_camera].attributes.calibration_points;
			}
